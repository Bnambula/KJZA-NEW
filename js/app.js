...
function doLogin() {
  ...
  if (!u) { msg.innerHTML = '<div class="alert alert-error">Invalid email or password.</div>'; return; }
  msg.innerHTML = '';
...
function doSignup() {
  ...
  if (!fn || !em || !pw) { msg.innerHTML = '<div class="alert alert-error">Please fill all required fields.</div>'; return; }
  if (KF.data.users.find(u => u.email === em)) { msg.innerHTML = '<div class="alert alert-error">Email already registered.</div>'; return; }
  if (pw.length < 6) { msg.innerHTML = '<div class="alert alert-error">Password must be at least 6 characters.</div>'; return; }
...
function updateAuthUI() {
  ...
  document.getElementById('admin-nav-item').classList.toggle('hidden', !(u && u.role === 'admin'));
  if (u) {
    document.getElementById('user-pill-name').textContent = u.name.split(' ')[0];
  }
}
    ...
    return `<div class="product-card">
      <div class="product-img">
        ...
        <div class="prod-status-badge">
          ${out ? '<span class="badge badge-red">Out of Stock</span>' : '<span class="badge badge-green">Fresh</span>'}
        </div>
      </div>
      <div class="product-body">
        <div class="product-name">${p.name}</div>
        <div class="product-cat">${KF.catEmoji(p.catId)} ${KF.catName(p.catId)}</div>
        <div class="product-price">${KF.fmt(p.price)} <small>/ ${p.unit}</small></div>
      </div>
      <div class="product-footer">
        <span class="stock-note ${out ? 'text-red' : low ? 'text-amber' : 'text-muted'}">
          ${out ? '✕ Out of stock' : low ? '⚠ Only ' + p.stock + ' left' : '✓ In stock'}
        </span>
        <button class="add-cart-btn" ${out ? 'disabled' : ''} onclick="addToCart(${p.id})">+ Add</button>
      </div>
    </div>`;
  }).join('');
}
...
function coNext() {
  if (coCurrentStep === 1) {
    ...
    if (!document.getElementById('co-phone').value) { toast('Please enter your phone number', '⚠️'); return; }
    renderOrderSummary();
...
function renderCheckoutStep(n) {
  coCurrentStep = n;
  ['co-step-1','co-step-2','co-step-3'].forEach((id, i) => {
...
function renderOrderSummary() {
  ...
  const sub = KF.cartSubtotal();
  document.getElementById('order-summary-lines').innerHTML =
    KF.state.cart.map(i => {
      const p = KF.data.products.find(x => x.id === i.pid);
...
function placeOrder() {
  ...
  const order = {
    id: oid,
    customer: KF.state.currentUser ? KF.state.currentUser.name : (document.getElementById('co-fname').value || 'Guest'),
  ...
  document.getElementById('placed-order-id').textContent = oid;
  document.getElementById('placed-zone').textContent = KF.state.selectedZone;
  document.getElementById('placed-total').textContent = KF.fmt(sub + fee);
  KF.state.cart = [];
  updateCartUI();
  renderProductGrid();
}
